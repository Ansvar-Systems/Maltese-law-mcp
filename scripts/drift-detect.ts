#!/usr/bin/env tsx
/**
 * Drift detection for Maltese Law MCP.
 *
 * Checks if upstream source content has changed since last ingestion.
...
async function main(): Promise<void> {
  console.log('Maltese Law MCP — Drift Detection');
  console.log('=====================================\n');
  ...
  for (const hash of fixture.provisions) {
    ...
    try {
      const response = await fetch(hash.upstream_url, {
        headers: { 'User-Agent': 'Maltese-Law-MCP/1.0 drift-detect' },
      });
