#!/usr/bin/env tsx
/**
 * Maltese Law MCP — Data Freshness Checker
 *
...
const PORTAL_URL = 'https://legislation.mt';
const PORTAL_NAME = 'Legislation Malta';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/maltese-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Maltese Law MCP — Data Freshness Check');
