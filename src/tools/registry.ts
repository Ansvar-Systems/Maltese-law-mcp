/**
 * Tool registry for Maltese Law MCP Server.
...
import { getMalteseImplementations, type GetMalteseImplementationsInput } from './get-maltese-implementations.js';
...
const LIST_SOURCES_TOOL: Tool = {
  ...
  description:
    ...
    'including the Laws of Malta (Ministry of Justice). ' +
    ...
    'Call this FIRST when you need to understand what Maltese legal data this server covers.',
  inputSchema: { type: 'object', properties: {} },
};
...
export const TOOLS: Tool[] = [
  {
    ...
    description:
      'Search Maltese statutes and regulations by keyword using full-text search (FTS5 with BM25 ranking). ' +
  ...
  },
  {
    name: 'get_provision',
    description:
      'Retrieve the full text of a specific provision (section) from an Maltese statute. ' +
  ...
  },
  {
    ...
    description:
      'Validate an Maltese legal citation against the database — zero-hallucination check. ' +
  ...
  },
  {
    ...
    description:
      'Build a comprehensive set of citations for a legal question by searching across all Maltese statutes simultaneously. ' +
      'Returns aggregated results from multiple relevant provisions, useful for legal research on a topic. ' +
      'Use this for broad legal questions like "What are the penalties for data breaches in Maltese?" ' +
  ...
  },
  {
    ...
    description:
      'Format an Maltese legal citation per standard conventions. ' +
  ...
  },
  {
    name: 'check_currency',
    description:
      'Check whether an Maltese statute or provision is currently in force, amended, repealed, or not yet in force. ' +
  ...
  },
  {
    name: 'get_eu_basis',
    description:
      'Get the EU legal basis that an Maltese statute references or aligns with. ' +
      'Maltese is an EU member state and Maltese laws implement EU directives/regulations ' +
      'where applicable. ' +
      'Returns EU document identifiers, reference types, and implementation status.',
    inputSchema: {
      type: 'object',
      properties: {
        document_id: { type: 'string', description: 'Maltese statute identifier.' },
        include_articles: {
          type: 'boolean',
  ...
  {
    name: 'get_maltese_implementations',
    description:
      'Find all Maltese statutes that reference or align with a specific EU directive or regulation. ' +
      'Given an EU document ID (e.g., "regulation:2016/679" for GDPR), returns matching Maltese statutes. ' +
      'Maltese transposes EU directives into national law as an EU Member State.',
    inputSchema: {
      type: 'object',
      properties: {
        eu_document_id: {
  ...
  },
  {
    ...
    description:
      'Search for EU directives and regulations that are referenced by Maltese legislation. ' +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        has_maltese_implementation: {
          type: 'boolean',
          description: 'If true, only return EU documents referenced by Maltese legislation.',
        },
  ...
  },
  {
    ...
    description:
      'Get the EU legal basis for a SPECIFIC provision within an Maltese statute. ' +
  ...
  },
  {
    ...
    description:
      'Check EU alignment status for an Maltese statute or provision. ' +
      ...
      'Maltese is an EU Member State; this checks implementation status and cross-references.',
    inputSchema: {
      type: 'object',
      properties: {
        document_id: { type: 'string', description: 'Maltese statute identifier.' },
    ...
    try {
      ...
      switch (name) {
        ...
        case 'get_eu_basis':
          ...
          break;
        case 'get_maltese_implementations':
          result = await getMalteseImplementations(db, args as unknown as GetMalteseImplementationsInput);
          break;
