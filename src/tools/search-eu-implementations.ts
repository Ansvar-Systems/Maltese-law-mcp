/**
 * search_eu_implementations — Search EU directives/regulations referenced by Maltese legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_maltese_implementation?: boolean;
  limit?: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as maltese_statute_count
  ...
  if (input.has_maltese_implementation) {
    sql += ' HAVING maltese_statute_count > 0';
  }
