/**
 * get_maltese_implementations — Find Maltese statutes that reference a specific EU directive/regulation.
 */

import type Database from '@ansvar/mcp-sqlite';
import { generateResponseMetadata, type ToolResponse } from '../utils/metadata.js';

export interface GetMalteseImplementationsInput {
  eu_document_id: string;
  primary_only?: boolean;
  in_force_only?: boolean;
}

export interface MalteseImplementationResult {
  document_id: string;
...
export async function getMalteseImplementations(
  db: InstanceType<typeof Database>,
  input: GetMalteseImplementationsInput,
): Promise<ToolResponse<MalteseImplementationResult[]>> {
  try {
  ...
  const rows = db.prepare(sql).all(...params) as MalteseImplementationResult[];
